...
Enable it via :func:`~ensembl.utils.docs.config.build_config`::
...
def _build_markdown_table(entry_points: dict[str, str]) -> str:
    ...
    Returns:
        A complete Markdown table, or a short italicised notice when ``entry_points`` is empty.
...
def _inject_table(usage_path: Path, table_md: str) -> None:
    ...
    If the sentinels are not found the table and the sentinels are appended to the end of the file with
    a preceding blank line, so the extension is safe to add to a page that has not yet been prepared.
    ...
    if _REGION_RE.search(original):
        ...
        logger.warning(
            "entrypoints_table: sentinels not found in '%s'. Appending the table and a templated "
            "section at the end of the file.",
            usage_path,
        )
        cli_section = (
            "\n\n## CLI reference\n\nThe following commands are installed as entry points to ease "
            f"handling common tasks:\n\n{replacement_block}\n"
        )
...
def _on_builder_inited(app: Sphinx) -> None:
    ...
    target_rel: str = app.config.entrypoints_target_file  # type: ignore[attr-defined]
    toml_rel: str = app.config.entrypoints_toml_file  # type: ignore[attr-defined]
